...
do
	local last_exec = _ENV.AllBlack_execute_debounce
	if last_exec and (tick() - last_exec) <= 5 then
		return nil
	end
	_ENV.AllBlack_execute_debounce = tick()
end
...
do
	...
	if not _ENV.AllBlack_teleport_queue and type(queueteleport) == "function" then
		_ENV.AllBlack_teleport_queue = true
...
local function CreateMessageError(Text)
	if _ENV.AllBlack_error_message then
		_ENV.AllBlack_error_message:Destroy()
	end
